...
export default function RecommendationsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      {/* ── Page header ──────────────────────────────────────────────────── */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-stone-900 mb-1">{t('rec.title')}</h1>
        <p className="text-stone-500">{t('rec.subtitle')}</p>
      </div>
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-32">
          ...
          <p className="text-stone-500 mb-4">{t('rec.noHistory')}</p>
          <a
            href="/analyze"
        ...
        <>
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-3 text-sm text-white/80 mb-4">
                  ...
                  {(analysis as any)?.difficulty_estimate && (
                    <span>⭐ {'★'.repeat((analysis as any).difficulty_estimate)}</span>
                  )}
                </div>
                <button
                  onClick={generate}
                ...
                >
                  ...
                  ) : recs ? t('rec.regenBtn') : t('rec.generateBtn')}
                </button>
                {genError && (
          ...
          {recs && (
            <div>
              <div className="flex items-center gap-2 mb-6">
                <h2 className="text-lg font-bold text-stone-900">{t('rec.heading')}</h2>
                <span className="text-xs text-stone-400">{t('rec.byAI')}</span>
              </div>
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <RecCard icon="🧶" title={t('rec.card.fabric')}>
                  <div className="space-y-3 text-sm">
                    <FabricRow label={t('rec.fabric.rec')} labelColor="#2E5E4E" value={recs.fabric.primary} />
                    <FabricRow label={t('rec.fabric.alt')} labelColor="#6B7280" value={recs.fabric.alternative} />
                    <FabricRow label={t('rec.fabric.avoid')} labelColor="#EF4444" value={recs.fabric.avoid} />
                  </div>
                </RecCard>
                ...
                {/* 7. Mood Patterns — spans full width */}
          ...
          {!recs && !generating && selected && (
            <div className="text-center py-16 text-stone-400">
              ...
              <p className="text-stone-500 mb-2">{t('rec.emptyHint')}</p>
              <p className="text-stone-400 text-sm">{t('rec.emptyHint2')}</p>
            </div>
          )}
        </>
      )}
